...
def ft_tqdm(lst: range) -> None:
    ...
    a progress bar in the terminal. The progress bar shows completion percentage,
    elapsed time, estimated time remaining, and iteration speed.
    ...
    for i, item in enumerate(lst, start=1):
        ...
        bar_prog_data = int(act_bar_width * progress_ratio(i, len(lst)))
        lap_time = format_time(lap_time)
        left_time = format_time((len(lst) - i) / speed)
        bar_percent = f'{progress_ratio(i, len(lst)) * 100:>3.0f}%|'
        bar_progress = f'{'█' * bar_prog_data:<{act_bar_width}}'
        bar_data = f'| {i}/{len(lst)} [{lap_time}<{left_time}, {speed:0.2f}it/s]'
